...
virtual_router_id=${KEEPALIVED_ROUTER_ID:-12}
...
perl -p -i -e "s/\{\{ virtual_router_id \}\}/$virtual_router_id/" $PATH_KEEPALIVED_CONF
...
#    /usr/sbin/keepalived --dont-fork --dump-conf --log-console --log-detail --vrrp &
    /usr/sbin/keepalived --dont-fork --dump-conf --log-console --vrrp &
